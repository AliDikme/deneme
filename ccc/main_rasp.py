...
class App:
    ...
    def processTheAuido(self):
        ...
        if(command2 == normal):
            print("hello")
            os.system("tmux new -d 'sudo rfcomm connect hci0  00:18:EF:00:04:38'")

            ser = serial.Serial(
                port='/dev/rfcomm0',
                baudrate=9600)
            count=0
            global stateOfSensor
            stateOfSensor = []
            while(count < 5):
                received_data = ser.readline()
                sleep(0.3)
                count=count + 1
                stateOfSensor.append(received_data)
            print(stateOfSensor)
            totalString = stateOfSensor[0] + " " + stateOfSensor[1] + " " + stateOfSensor[2] + " " + stateOfSensor[3] + " " + stateOfSensor[4]
            tts = gtts.gTTS(
                totalString, lang="tr")
            # save the audio file
            tts.save("hello.mp3")
        ...
        elif(isSubstring(command3, normal)):
            
            remiderApp = converter.StateManager()
            remiderApp.initStateCheck()
            resultValue= converter.async_result.get()
            hade = converter.Reminder("reminder", datetime.datetime(
                2022, 6, 15, 13, 45), 1, "tansiyon ilaçını al")
            
            tts = gtts.gTTS(
                f"{hade.type} tipindeki hatırlatıcınızın mesajı : {hade.message}", lang="tr")
            # save the audio file
            tts.save("hello.mp3")
            # play the audio file
            # playsound("hello.mp3")
        ...
        elif(isSubstring(command4, normal)):
